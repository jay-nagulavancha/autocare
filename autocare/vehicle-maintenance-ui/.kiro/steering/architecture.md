...
|---|---|---|
| `JWT_SECRET` | auth-service, maintenance-service | Shared HS256 signing key |
| `JWT_EXPIRATION_MS` | auth-service | Token TTL in milliseconds |
| `DB_HOST` | auth-service, maintenance-service | MySQL hostname |
| `DB_PORT` | auth-service, maintenance-service | MySQL port (default 3306) |
| `DB_NAME` | auth-service, maintenance-service | Schema name (different per service) |
| `DB_USERNAME` | auth-service, maintenance-service | MySQL user |
| `DB_PASSWORD` | auth-service, maintenance-service | MySQL password |
...
mysql (health check: mysqladmin ping)
  └── auth-service        (depends_on: mysql, condition: service_healthy)
  └── maintenance-service (depends_on: mysql, condition: service_healthy)
        └── ui            (depends_on: auth-service, maintenance-service)
```
