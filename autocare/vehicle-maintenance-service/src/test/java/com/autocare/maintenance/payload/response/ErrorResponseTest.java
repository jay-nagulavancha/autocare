...
public class ErrorResponseTest {
    ...
    @Test
    void testGetErrorsWithEmptyListReturnsCopy() {
    ...
    @Test
    void testMultipleErrorsPreserveOrder() {
        List<ErrorResponse.FieldError> errors = Arrays.asList(
                new ErrorResponse.FieldError("a", "error_a"),
                new ErrorResponse.FieldError("b", "error_b"),
                new ErrorResponse.FieldError("c", "error_c")
        );
        ...
        assertEquals("a", retrieved.get(0).getField());
        assertEquals("b", retrieved.get(1).getField());
        assertEquals("c", retrieved.get(2).getField());
    }
    ...
    @Test
    void testSetErrorsDoesNotStoreOriginalReference() {
        ...
        // The stored list should be a copy, not the same reference
        List<ErrorResponse.FieldError> retrieved = errorResponse.getErrors();
        assertNotSame(originalErrors, retrieved);
    }
}